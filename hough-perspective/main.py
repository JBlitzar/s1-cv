...
for vote in tqdm(new_votes, leave=False):
    ...
    for real_vote in votes:
        if (vote[0] - real_vote[0]) ** 2 + (vote[1] - real_vote[1]) ** 2 < (
            step_size / 2
        ) ** 2:
...
all_points = np.vstack(
    [[[0, 0], [hough_vis.shape[1], hough_vis.shape[0]]], vanishing_points]
)
...
canvas[
    offset_y : offset_y + hough_vis.shape[0], offset_x : offset_x + hough_vis.shape[1]
] = hough_vis
...
for vp in vanishing_points:
    ...
    if (
        abs_angle_diff(angle, direction) > np.pi / 2
        and abs_angle_diff(get_angle(vp2), angle) > np.pi / 2
        and not is_on_screen
    ):
...
model_points *= 500.0
model_points += np.array([0, 300, 600])
