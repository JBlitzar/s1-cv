...
def obtain_featuredescriptors(path):
    ...
    feature_descriptors = [FeatureDescriptor(f[0][0], f[0][1], desc) for f, desc in zip(features, descriptors)]
...
def match_next_image(descriptors1, descriptors2):
    ...
    def attempt_get_homography():
        ...
        H, _ = cv2.findHomography(
            np.array([[p.x, p.y] for p in pts1]),
            np.array([[p.x, p.y] for p in pts2]))
        if H is None:
...
def match_next_image_RANSAC_builtin(descriptors1, descriptors2):
    ...
    H, _ = cv2.findHomography(
        ...
        np.array([[p.x, p.y] for p in [dp1.get_closest_match(descriptors2) for dp1 in descriptors1]]), cv2.RANSAC)
